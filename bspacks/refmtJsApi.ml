...
let flowLocationToJsObj (loc: Refmt_api.Loc.t) =
  let (start_line,start_char) = (loc.start.line, loc.start.column) in
  let (end_line,end_char) = (loc._end.line, loc._end.column) in
  (* The right way of handling ocaml syntax error locations. Do do this at home
    copied over from
    https://github.com/BuckleScript/bucklescript/blob/2ad2310f18567aa13030cdf32adb007d297ee717/jscomp/super_errors/super_location.ml#L73
  *)
...
let getBindings file code =
  ...
  try
    ...
    |> printRE
  with
  | Refmt_api.Parse_error.Error ((location, err)::tl) ->
    let jsLocation = flowLocationToJsObj location in
    let jsError =
      Js.Unsafe.obj [|
        ("message", Js.Unsafe.inject (Js.string (Refmt_api.Parse_error.PP.error err)));
        ("location", Js.Unsafe.inject jsLocation);
      |]
    in
    Js.Unsafe.fun_call throwAnything [|Js.Unsafe.inject jsError|]
